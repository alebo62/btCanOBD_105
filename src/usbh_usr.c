...
void USART_Configuration(uint16_t);
...
//	NVIC_EnableIRQ(TIM7_IRQn);
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 ;// enable
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIOB->ODR &= ~0x01; // off transistor
	
	
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1, ENABLE);
	TIM1->CNT = 1;
	TIM1->PSC = 36000 - 1;// 1ms
	TIM1->ARR = 20;  // 20 msec
...
//        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2 | GPIO_Pin_3 | GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_4 | GPIO_Pin_5;
//        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
//        GPIO_Init(GPIOA, &GPIO_InitStructure);
...
void TIM4_IRQHandler(void) //  tx press timer
{
...
void USART_Configuration(uint16_t brr)
{
    USART_InitTypeDef USART_InitStructure;
