 ...
 ******************************************************************************
 --добавлена проверка количества каналов
 */
...
//#define HSE_8MHZ установить в опциях С/C++ если кварц = 8 MHz
//#define DEBUG 
        ...
        if (rx[0]) {
            ...
            case S_CHNG_NOTIFIC:
                if ((rx[13] == 0xC4) && (rx[14] == 0x80))
                {
                    ...
                    call_req.pep.pld[0] =
                            ((call_type[channel_tx] == 0x67) || (call_type[channel_tx] == 0x47)) ? 1 : 0; //  set channel for ptt
            ...
            case TXT_NOTIFIC:
                ...
                else if ((rx[13] == 0xC4) && (rx[14] == 0x80))// change channel reply
                {
                    current_chan = rx[27];
                    enable_ptt = 1;
                    debounce = 0;
...
void extend_key_process(void)
{
    if (!press_ptt && (keys_state != 0x3F) && !debounce && !enable_ptt && !chan_busy && tx_enable)
    {
    ...
    if (debounce == 2) {
        if (keys_state == keys_debounce)
        {
            ...
            if (current_chan != chan_number[channel_tx] )// если надо переключить канал
                    //&& (state == TXT_NOTIFIC))
            {
							  call_req.pep.pld[0] = ((call_type[channel_tx] == 0x67) || (call_type[channel_tx] == 0x47)) ? 1 : 0;
							  memcpy((char*) call_req.pep.pld + 1,(char*) (call_number + channel_tx), 4); // check!!!!!!!!!!!!!!!!
                busy_channel = channel_tx + 1;
                ...
                USBH_BulkSendData(&USB_OTG_Core, tx, tx[9], 7); //
                USBH_BulkReceiveData(&USB_OTG_Core, rx, 64, 5);
                enable_ptt = 0; // ждем когда переключится канал
            }
            else
            {
                enable_ptt = 1; // это передача
            }
        }
				else
				{
					bad_channel:					debounce = 0;  // сброс в нач. состояние
				}
    }
    ...
    if ((state == WAIT_STOP_CALL_REQ) && ((GPIOC->IDR & 0x3F) == 0x3F) && press_key)// после нажатия отпустили кнопку каналов
    {
        //GPIOC->ODR &= ~LEDmic;
        press_key = 0;
        packet_num++;
    ...
    // read ptt
    if (!press_key && (((GPIOC->IDR) & PDLkey) == 0) && !press_ptt && !ptt_debounce && !chan_busy && tx_enable)
    {
			  tx_enable = 0;
			  TIM4->CR1 |= TIM_CR1_CEN;
    ...
    if ((state == WAIT_STOP_CALL_REQ) && ((GPIOC->IDR) & PDLkey)&& press_ptt)
    {
        GPIOC->ODR &= ~LEDmic;
        press_ptt = 0;
        packet_num++;
        ...
        ptt_release_req.packet_num[1] = packet_num & 0xFF;
        memcpy(tx, (char*) (&ptt_release_req), 17);
        memcpy(tx + 17, (char*) (ptt_release_req.pep.pld), ptt_release_req.pep.num_of_bytes[0]);
        memcpy(tx + 17 + ptt_release_req.pep.num_of_bytes[0], (char*) (&ptt_release_req) + 21, 2);
        checksum(tx);
        USBH_BulkSendData(&USB_OTG_Core, tx, tx[9], 7);
        state = WAIT_STOP_CALL_REQ;
    }
...
void read_data(void) 
{
    ...
    for (int i = 0; i < channels; i++)
    {
			  chan_number[i] = (*(__IO uint32_t*)p_flash);// номер канала
        p_flash += 4;
        call_type[i] = (*(__IO uint32_t*)p_flash); //  тип вызова
        p_flash += 4;
        call_number[i] = (*(__IO uint32_t*)p_flash);// номер абонента
        p_flash += 4;
    }
    FLASH_Lock();
}
...
void prog_mode(void)
{
    if (!GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_1))
      ...
      func_key = 1;
		...
		FLASH_Unlock();
    FLASH_ErasePage(MY_FLASH_PAGE_ADDR_MODE );
    uint32_t *source_addr = (void *) &func_key;
    uint32_t *dest_addr = (uint32_t *) MY_FLASH_PAGE_ADDR_MODE;
    FLASH_ProgramWord((uint32_t) dest_addr, *source_addr);
    FLASH_Lock();
...
{
    FLASH_Unlock();
    uint32_t p_flash = MY_FLASH_PAGE_ADDR_MODE;
    func_key = (*(__IO uint32_t*)p_flash);
	  FLASH_Lock();
	  if(!func_key)
			GPIOA->ODR |= 0x20;
		else
...
void normal_keys_process(void)
{
    if ((keys_state != 0x3F) && !debounce && tx_enable)
		{
			  tx_enable = 0;
			  TIM4->CR1 |= TIM_CR1_CEN;
    ...
    if (debounce == 2)
    {
        if (keys_state == keys_debounce)
        {
            ...
            GPIOA->ODR |= LEDs[channel_tx];
            call_req.pep.pld[0] =
                    ((call_type[channel_tx] == 0x67) || (call_type[channel_tx] == 0x47)) ? 1 : 0;
            memcpy((char*) call_req.pep.pld + 1,
                   (char*) (call_number + channel_tx), 4); // check!!!!!!!!!!!!!!!!
					}
          if (current_chan != chan_number[channel_tx] && (channel_tx <= (channels - 1)))
            {
                packet_num++;
                zone_ch.packet_num[0] = packet_num >> 8; // audio route rx tx  page232
                zone_ch.packet_num[1] = packet_num & 0xFF;
    ...
    // read ptt
    if ((((GPIOC->IDR) & PDLkey) == 0) && !press_ptt && !ptt_debounce && !chan_busy && tx_enable)
    {   
			  tx_enable = 0;
			  TIM4->CR1 |= TIM_CR1_CEN;
